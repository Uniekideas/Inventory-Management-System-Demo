...
function SignUp() {
  ...
  const { handleSigUpSubmit, signInIsLoading, sigUpResponse, sigUpError, setSigUpResponse, setSigUpError } = useContext(AuthenticationContext);

  useEffect(() => {
    if (!signInIsLoading && sigUpResponse) {
      navigate("/Login", { state: { message: sigUpResponse } });
 ...
 const handleLoadingClick = () => {
  if (signInIsLoading || !signInIsLoading && (!sigUpError && !sigUpResponse)) {
        ...
        />
          ...
          <Form onSubmit={handleSubmit}>
            <Form.Group >
              <Row>
                <Col md={7}>
                  <Form.Select size="lg" className="mb-3 inputSeleteField" name="role" required>
            ...
            >
                          {
              buttonLoading? <FontAwesomeIcon icon={faSpinner} spin size="2x" /> :'Sign Up'
              }
            </Button>
            <p>
              Are you a new user?{" "}
